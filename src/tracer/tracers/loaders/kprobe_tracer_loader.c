...
{
    /* Silence unless LIBBPF_VERBOSE=1 */
...
/* Find first loaded kprobe program whose name has the given prefix */
static int find_kprobe_prog_fd_by_name_prefix(const char *name_prefix)
{
    char pin_path[256];
    struct bpf_link_info linfo = {};
    __u32 link_info_len = sizeof(linfo);

    /* Prefer pinned link for exact match; survives program name changes. */
                       ...
                       const struct tracer_metrics_cfg *metrics)
{
    ...
    int target_fd = find_kprobe_prog_fd_by_name_prefix(target_name);
    if (target_fd < 0) {
        fprintf(stderr, "    [kprobe-tracer] target '%s' not found; ensure kprobe program is loaded and named accordingly.\n", target_name);
        return 0;
    }
    ...
    if (!fentry_prog || !fexit_prog) {
        fprintf(stderr, "    [kprobe-tracer] missing tracer programs in %s\n", tracer_object_path);
    ...
    if (err) {
        fprintf(stderr, "    [kprobe-tracer] load failed for %s: %d\n", tracer_object_path, err);
        bpf_object__close(obj);
        close(target_fd);
        return -1;
    }

    /* Metrics configuration (only time & ret used for kprobe) */
    ...
    if (cfg_map) {
        ...
        cfg.enable_op = 0;  // not used for kprobe
        cfg.target_prog_id = target_prog_id;
        fprintf(stderr, "    [kprobe-tracer] Writing target_prog_id=%u to config map\n", target_prog_id);
        __u32 k = 0;
        if (bpf_map_update_elem(cfg_fd, &k, &cfg, BPF_ANY) != 0) {
            fprintf(stderr, "    [kprobe-tracer] warning: failed metrics_cfg update for %s\n", target_name);
        }
    } else {
        fprintf(stderr, "    [kprobe-tracer] warning: metrics_cfg map absent in %s\n", tracer_object_path);
    }
    ...
    if (need_fentry) {
        ...
        if (libbpf_get_error(le)) {
            fprintf(stderr, "    [kprobe-tracer] fentry attach failed (%s)\n", fentry_name);
            le = NULL;
        } else {
            attached++;
        }
    ...
    if (need_fexit) {
        ...
        if (libbpf_get_error(lx)) {
            fprintf(stderr, "    [kprobe-tracer] fexit attach failed (%s)\n", fexit_name);
            lx = NULL;
        } else {
            attached++;
        }
    ...
    close(target_fd);
    if (attached > 0)
        printf("    [kprobe-tracer] attached: target=%s fentry=%s fexit=%s\n", target_name,
               need_fentry?"yes":"no", need_fexit?"yes":"no");
    else {
        printf("    [kprobe-tracer] no tracer links attached for %s\n", target_name);
        bpf_object__close(obj);
    }
    return attached;
}
