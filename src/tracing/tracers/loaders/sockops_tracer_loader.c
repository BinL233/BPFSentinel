...
/* Find loaded sockops program by name prefix */
static int find_sockops_prog_fd_by_name_prefix(const char *name_prefix)
{
    __u32 id = 0, next_id;
    ...
    while (bpf_prog_get_next_id(id, &next_id) == 0) {
        ...
        if (bpf_obj_get_info_by_fd(fd, &info, &info_len) == 0) {
            if (info.type == BPF_PROG_TYPE_SOCK_OPS) {
                        ...
                        const struct tracer_metrics_cfg *metrics)
{
    ...
    int target_fd = find_sockops_prog_fd_by_name_prefix(target_name);
    if (target_fd < 0) {
        fprintf(stderr, "    [sockops-tracer] target '%s' not found, skipping.\n", target_name);
        return 0;
    }
    ...
    if (!fentry_prog || !fexit_prog) {
        fprintf(stderr, "    [sockops-tracer] missing tracer programs in %s\n", tracer_object_path);
    ...
    if (err) {
        fprintf(stderr, "    [sockops-tracer] load failed for %s: %d\n", tracer_object_path, err);
        bpf_object__close(obj);
        close(target_fd);
        return -1;
    }

    /* Configure metrics map (enable_op included) */
    ...
    if (cfg_map) {
        ...
        fprintf(stderr, "    [sockops-tracer] warning: metrics_cfg map absent in %s\n", tracer_object_path);
    }

    int attached = 0;
    struct bpf_link *le = NULL, *lx = NULL;
    int need_fentry = (metrics->want_time || metrics->want_ret || metrics->want_pkt_len || metrics->want_op);
    int need_fexit  = (metrics->want_time || metrics->want_ret || metrics->want_pkt_len || metrics->want_op);

    if (need_fentry) {
        le = bpf_program__attach_trace(fentry_prog);
        if (libbpf_get_error(le)) {
            fprintf(stderr, "    [sockops-tracer] fentry attach failed (%s)\n", fentry_name);
            le = NULL;
        } else {
            attached++;
        }
    ...
    if (need_fexit) {
        ...
        if (libbpf_get_error(lx)) {
            fprintf(stderr, "    [sockops-tracer] fexit attach failed (%s)\n", fexit_name);
            lx = NULL;
        } else {
            attached++;
        }
    ...
    if (attached > 0)
        printf("    [sockops-tracer] attached: target=%s fentry=%s fexit=%s\n", target_name,
               need_fentry?"yes":"no", need_fexit?"yes":"no");
    else {
        printf("    [sockops-tracer] no tracer links attached for %s\n", target_name);
        bpf_object__close(obj);
    }
